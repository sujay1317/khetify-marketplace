...
const AdminDashboard: React.FC = () => {
  ...
  const [stats, setStats] = useState({ users: 0, products: 0, orders: 0, pendingProducts: 0 });
  ...
  const getInitials = (name: string | undefined | null) => { if (!name) return 'A'; return name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2); };
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div className="flex items-center gap-4">
                <Avatar className="w-14 h-14 sm:w-16 sm:h-16 ring-2 ring-purple-500/30">
                  ...
                  <AvatarFallback className="bg-purple-600 text-white text-lg font-semibold">{getInitials(profile?.full_name)}</AvatarFallback>
                </Avatar>
                <div>
                  <div className="flex items-center gap-2"><CardTitle className="text-xl sm:text-2xl font-heading">Welcome, {profile?.full_name?.split(' ')[0] || 'Admin'}! 👑</CardTitle><Badge variant="destructive">Admin</Badge></div>
                  <p className="text-sm text-muted-foreground mt-1">Manage your platform and monitor activity</p>
                </div>
              </div>
              <Link to="/admin"><Button className="gap-2 bg-purple-600 hover:bg-purple-700"><Settings className="w-4 h-4" />Admin Panel</Button></Link>
            </div>
          </CardHeader>
